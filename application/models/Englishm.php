...
class EnglishM extends CI_Model {
	...
	function get_campaigns_data_state($state)
	{
		...
		$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from district where id = a.distid )eng_distname,(select engname from constituency where id = a.constid )eng_constname,(select engname from parties where id=a.partyid )eng_partyname,(select engname from campaigners where id = a.campainerid)eng_campaigner,(select telname from state where id = a.stateid)tel_statename,(select telname from district where id = a.distid )tel_distname,(select telname from constituency where id = a.constid )tel_constname,(select telname from parties where id=a.partyid )tel_partyname,(select telname from campaigners where id = a.campainerid)tel_campaigner from campaigns a where a.stateid = (select id from state where engname like "'.$state.'")  order by a.campaign_date desc' );
		return $query->result_array();
	}
	...
	function get_pre_result_by_district($district)
	{
		...
		$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from parties where id = a.partycode)eng_partyname,count(a.partycode)as votes,(select telname from state where id = a.stateid)tel_statename,(select telname from district where id = a.distid )tel_distname,(select telname from constituency where id = a.constid)tel_constname,(select telname from parties where id=a.partycode)tel_partyname,(select telname from candidates where id=a.candidatecode)tel_candidatename,(select symbolpath from parties where id = a.partycode)symbol from pre_results a where a.distid = (select id from district where engname like "'.$district.'") and a.status=1   and a.year=2014 and status =1  group by a.partycode,a.year order by a.year desc,votes desc; ');
		return $query->result_array();
	}
	...
	function get_seats_data_distid($district)
	{
		...
		$query = $this->db->query(' select a.*,group_concat(a.noofvotes order by a.status)noofvotes,(select engname from state where id = a.stateid)eng_statename,(select engname from district where id = a.distid )eng_distname,(select engname from constituency where id = a.constid)eng_constname,group_concat((select engname from parties where id=a.partycode) order by a.status)eng_partyname,group_concat((select engname from candidates where id=a.candidatecode) order by a.status)eng_candidatename,(select telname from state where id = a.stateid)tel_statename,(select telname from district where id = a.distid )tel_distname,(select telname from constituency where id = a.constid)tel_constname,group_concat((select telname from parties where id=a.partycode) order by a.status)tel_partyname,group_concat((select telname from candidates where id=a.candidatecode) order by a.status)tel_candidatename,(select noofvoters from constituency where id = a.constid)totalvotes,group_concat((select symbolpath from parties where id = a.partycode) order by a.status)symbol from pre_results a where a.distid = (select id from district where engname like "'.$district.'")   and a.year!=0     group by a.constid,a.year order by a.constid,a.year desc,a.noofvotes desc' );
		return $query->result_array();
	}
	...
	{
		$query  = $this->db->query('select a.telname,(select telname from parties where id = a.partycode)party,(select symbolpath from parties where id = a.partycode)symbol,(select telname from constituency where id = a.constid)const from candidates a where a.year= 2018 and a.distid = (select id from district where engname like "'.$district.'")');
		return $query->result_array();
	}
	...
	function get_seats_data_const($const)
	{
		...
		$query = $this->db->query('  select a.*,group_concat(a.noofvotes order by a.status)noofvotes,(select engname from state where id = a.stateid)eng_statename,(select engname from district where id = a.distid )eng_distname,(select engname from constituency where id = a.constid)eng_constname,group_concat((select engname from parties where id=a.partycode) order by a.status)eng_partyname,group_concat((select engname from candidates where id=a.candidatecode) order by a.status)eng_candidatename,(select telname from state where id = a.stateid)tel_statename,(select telname from district where id = a.distid )tel_distname,(select telname from constituency where id = a.constid)tel_constname,group_concat((select telname from parties where id=a.partycode) order by a.status)tel_partyname,group_concat((select telname from candidates where id=a.candidatecode) order by a.status)tel_candidatename,(select noofvoters from constituency where id = a.constid)totalvotes,group_concat((select symbolpath from parties where id = a.partycode) order by a.status)symbol from pre_results a where a.constid = (select id from constituency where engname like "'.$const.'" and status = 1) and a.year != 0 and a.is_active = 1  group by a.constid,a.year order by a.constid,a.year desc,a.noofvotes desc' );
		return $query->result_array();
	}
	...
	function get_previous_election_result($const)
	{
		...
		$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from parties where id = a.partycode)eng_partyname,count(a.noofvotes)as votes ,(select telname from state where id = a.stateid)tel_statename,(select telname from district where id = a.distid )tel_distname,(select telname from constituency where id = a.constid)tel_constname,(select telname from parties where id=a.partycode)tel_partyname,(select telname from candidates where id=a.candidatecode)tel_candidatename,(select symbolpath from parties where id = a.partycode)symbol from pre_results a where a.constid = (select id from constituency where engname like "'.$const.'" and status = 1)  and year !=0 and year = 2014 group by a.partycode,year order by noofvotes desc; ');
		return $query->result_array();
	}

	function get_candidates_by_const($const)
	{
		$query  = $this->db->query('select a.telname,(select telname from parties where id = a.partycode)party,(select telname from constituency where id = a.constid)const,(select symbolpath from parties where id = a.partycode)symbol from candidates a where a.year= 2018 and a.constid = (select id from constituency where engname like "'.$const.'" and status = 1) ');
		return $query->result_array();
	}
	...
	function get_seats_data_by_party($partyid,$state)
	{
		...
		$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)statename,(select engname from district where id = a.distid )distname,(select engname from constituency where id = a.constid)constname,group_concat((select engname from parties where id=a.partycode) order by a.status)eng_partyname,group_concat((select engname from candidates where id=a.candidatecode) order by a.status)eng_candidatename,(select telname from state where id = a.stateid)tel_statename,(select telname from district where id = a.distid )tel_distname,(select telname from constituency where id = a.constid)tel_constname,group_concat((select telname from parties where id=a.partycode) order by a.status)tel_partyname,group_concat((select telname from candidates where id=a.candidatecode) order by a.status)tel_candidatename,(select noofvoters from constituency where id = a.constid)totalvotes from pre_results a where a.partycode = "'.$partyid.'" and a.stateid = (select id from state where engname like "'.$state.'") order by a.year desc,a.noofvotes desc' );
		return $query->result_array();
	}
	...
	function get_campaigns_data_party($partyid,$state)
	{
		...
		$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from district where id = a.distid )eng_distname,(select engname from constituency where id = a.constid )eng_constname,(select engname from parties where id=a.partyid )eng_partyname,(select telname from parties where id=a.partyid )tel_partyname,(select engname from campaigners where id = a.campainerid)eng_campaigner from campaigns a where a.partyid ="'.$partyid.'" and a.stateid = (select id from state where engname like "'.$state.'") order by a.campaign_date desc' );
		return $query->result_array();
	}
	...
	{
		$this->db->select('a.*,(select engname from state where id = a.stateid)statename,(select engname from district where id = a.distid )distname,(select engname from constituency where id = a.constid )constname,(select engname from parties where id=a.partycode )partyname,')->from('candidates a');
	...
	function get_campaigns_data_candidate($candidateid,$state)
	{
		...
		$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from district where id = a.distid )eng_distname,(select engname from constituency where id = a.constid )eng_constname,(select engname from parties where id=a.partyid )eng_partyname,(select engname from campaigners where id = a.campainerid)eng_campaigner from campaigns a where a.constid = (select constid from candidates where id = '.$candidateid.') and a.partyid =(select partycode from candidates where id = '.$candidateid.' ) and a.stateid = (select id from state where engname like "'.$state.'") order by a.campaign_date desc' );
		return $query->result_array();
	}
	...
	function get_campaigners_details($rid)
	{
		if(!empty($rid))
		{
			$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from parties where id=a.partyid )eng_partyname,(select telname from parties where id=a.partyid )tel_partyname from campaigners a where a.id in ('.$rid.')' );
		}
		else
		{
			$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from parties where id=a.partyid )eng_partyname,(select telname from parties where id=a.partyid )tel_partyname from campaigners a ' );
		}
	...
	function get_election_schedule()
	{
		...
		$query = $this->db->query(' select a.*,(select engname from state where id = a.stateid)eng_statename,(select engname from district where id = a.distid )eng_distname,(select engname from constituency where id = a.constid )eng_constname from election_schedule a order by a.election_date desc' );
		return $query->result_array();
	}
	...
	{
		$this->db->select('a.*,(select telname from parties where id = a.partycode)telpartyname,(select telname from constituency where id = a.constid)telconstname')->from('candidates a')->where('a.star',1)->order_by('priority_order');
		$query = $this->db->get();
		return $query->result_array();
	}
	...
	function get_all_candidate_details()
	{
		...
		$query = $this->db->query('select a.img_path,a.engname,a.telname,a.alignid,a.alignname,(select telname from parties where id = a.partycode)party,(select symbolpath from parties where id = a.partycode)symbol,(select telname from constituency where id = a.constid)const from candidates a where a.year= 2018 ');
	...
	function get_pre_result_json_by_state($state)
	{
		...
		$query = $this->db->query(' select year,group_concat(tel_shortname order by year desc,seats desc)shortname,group_concat(eng_partyname order by year desc,seats desc)partyname,group_concat(seats order by year desc,seats desc)seats,group_concat(color_codes order by year desc,seats desc)color_codes from  pre_results_view_state where eng_statename like "'.$state.'" and year = 2014 and status = 1 group by eng_statename,year order by year desc,seats desc ');
		return $query->result_array();
	}
	...
	function get_vote_share_json_by_state($state)
	{
		...
		//$query = $this->db->query('select a.*,(select engname from state where id = a.stateid)statename,(select engname from parties where id = a.partycode)partyname,count(a.partycode)as votes from pre_results a where a.stateid = (select id from state where engname like "'.$state.'") and a.status=1 and a.year != 0 and a.year is not null group by partyname,year order by a.year desc,votes desc; ');
		$query = $this->db->query('select a.year,group_concat(distinct(b.telname) )partyname,group_concat(a.totalshare)voteshare from  voteshare a left join parties b on b.id = a.partycode where a.stateid = (select id from state where engname like "'.$state.'") and rtype = "state" group by a.stateid,a.year  ');
		return $query->result_array();
	}
	...
	function get_pre_result_json_by_district($district)
	{
		...
		//$query = $this->db->query('select a.*,(select engname from state where id = a.stateid)statename,(select engname from parties where id = a.partycode)partyname,count(a.partycode)as votes from pre_results a where a.stateid = (select id from state where engname like "'.$state.'") and a.status=1 and a.year != 0 and a.year is not null group by partyname,year order by a.year desc,votes desc; ');
		$query = $this->db->query('select year,group_concat(tel_shortname order by year desc,seats desc)shortname,group_concat(eng_partyname order by year desc,seats desc)partyname,group_concat(seats order by year desc,seats desc)seats,group_concat(color_codes order by year desc,seats desc)color_codes from  pre_results_view_district where eng_distname like "'.$district.'" and year in (2014)  order by year desc,seats desc');
		return $query->result_array();
	}
	...
	function get_pre_all_result_json_by_district($district)
	{
		...
		$query = $this->db->query('select eng_distname,eng_shortname,tel_shortname,sum(seats)seats from  pre_results_view_district where eng_distname like "'.$district.'" and year !=0 and status =1  group by eng_partyname order by seats desc');
		return $query->result_array();
	}
	...
	function get_pre_result_json_by_const($const)
	{
		...
		//$query = $this->db->query('select a.*,(select engname from state where id = a.stateid)statename,(select engname from parties where id = a.partycode)partyname,count(a.partycode)as votes from pre_results a where a.stateid = (select id from state where engname like "'.$state.'") and a.status=1 and a.year != 0 and a.year is not null group by partyname,year order by a.year desc,votes desc; ');
		//$query = $this->db->query(' select year,group_concat(eng_shortname order by seats desc)shortname,group_concat(eng_partyname order by seats desc)partyname,group_concat(seats order by seats desc)seats,group_concat(color_codes order by seats desc)color_codes from  pre_results_view where eng_constname like "'.$const.'" and year != 0 group by eng_partyname  order by seats desc');
		$query = $this->db->query('select eng_constname,eng_shortname,tel_shortname,sum(seats)seats from  pre_results_view where eng_constname like "'.$const.'" and year != 0  and status =1 group by eng_partyname  order by seats desc');
		return $query->result_array();
	}
